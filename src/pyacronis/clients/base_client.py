...
from pyacronis.exceptions import (
...
if TYPE_CHECKING:
    from pyacronis.types import RequestData, RequestMethod, RequestParams


class AcronisClient(ABC):
    ...
    def _reauthenticate(self) -> bool:
        """
        Attempt to refresh this client's credentials after an authentication
        failure and report whether the request should be retried.

        The base client uses static credentials it cannot refresh, so it returns
        False. Clients backed by expiring tokens override this to
        fetch a new token and return True.
        """
        return False
    ...
    def _make_request(
        ...
        *,
        ...
        # Remember whether we built the auth headers ourselves. If the caller
        # supplied explicit headers (e.g. the token-acquisition request), we must
        # not attempt to re-authenticate on a 401 or we would recurse forever on
        # bad credentials.
        used_managed_headers = not headers
        if used_managed_headers:
        ...
        if not response.ok:
            with contextlib.suppress(json.JSONDecodeError):
                ...
                if response.status_code == 400 and details.get("code") == "InvalidObject":
            ...
            if response.status_code == 401:
                # Our cached token may have expired early (clock skew) or been
                # revoked server-side. If we manage the auth headers, re-authenticate
                # once and retry with a fresh token before surfacing the failure.
                if used_managed_headers and not auth_retried and self._reauthenticate():
