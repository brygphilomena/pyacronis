...
class AcronisException(Exception):
...
class MalformedRequestException(AcronisException):
...
class ServerError(AcronisException):
    _code_explanation = "Internal Server Error"


class ObjectExistsError(AcronisException):
