...
from pyacronis.utils.helpers import parse_link_headers, parse_response_body
...
if TYPE_CHECKING:
    ...
    from pyacronis.types import RequestParams


TModel = TypeVar("TModel", bound="BaseModel")

if TYPE_CHECKING:
    from pyacronis.interfaces import IPaginateable


class PaginatedResponse(Generic[TModel]):
    """
    PaginatedResponse is a wrapper class for handling paginated responses from the
    Acronis API. It provides methods for navigating through the pages of the response
    and accessing the data contained within each page.

    The class is designed to work with AcronisEndpoint and its derived classes to
    ...
    def _initialize(
        ...
        Args:
            ...
            endpointmodel (AcronisEndpoint[TModel]): The endpointmodel associated with the response.
        ...
        try:
            if self.parsed_pagination_response is not None:
                # Acronis API gives us a handy response to parse for Pagination
